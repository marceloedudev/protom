...
public class OrderRepositoryDatabase implements OrderRepository {
    ...
    @Override
    public void updateStatusById(Long orderId, int status) {
        orderPanache.update("status = :status where id = :id", Parameters.with("status", status).and("id", orderId));
    }

    @Override
    public void deleteAll() {
        orderItemPanache.deleteAll();
        orderPanache.deleteAll();
    }
}
